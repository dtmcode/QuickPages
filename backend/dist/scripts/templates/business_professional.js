...
async function main() {
    ...
    await db.insert(website_builder_schema_1.wbGlobalTemplateSections).values({ pageId: leistungen.id, name: 'Header', type: 'hero', order: 0, content: { heading: 'Unsere Leistungen', subheading: 'GKV, PKV und Selbstzahlerleistungen', minimal: true }, styling: { backgroundColor: '#0284c7', textColor: '#ffffff' } });
    const [team] = await db.insert(website_builder_schema_1.wbGlobalTemplatePages).values({ templateId: t.id, name: 'Team', slug: '/team', isHomepage: false, order: 2 }).returning();
    await db.insert(website_builder_schema_1.wbGlobalTemplateSections).values({ pageId: team.id, name: 'Team', type: 'about', order: 0, content: { heading: 'Dr. Klaus Weber & Team', text: '<p>Dr. Klaus Weber, Zahnarzt und Oralchirurg, hat nach seinem Studium in Wien und einer Assistenzzeit in Zürich 2004 seine eigene Praxis in München eröffnet. Heute arbeiten 3 Zahnärztinnen und 6 Prophylaxeassistentinnen im Team.</p><p>Schwerpunkte: Implantologie, ästhetische Zahnheilkunde und Behandlung von Angstpatienten.</p>', imageUrl: 'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=800&h=500&fit=crop', highlights: ['Oralchirurg · Implantologe', 'Zahnarzt des Jahres 2023', 'Lachgas-Sedierung', 'Angstpatienten-Sprechstunde'] } });
    console.log('✅ business_professional — Zahnarzt Dr. Weber, München');
    await pool.end();
}
main().catch(err => { console.error('❌', err); process.exit(1); });
//# sourceMappingURL=business_professional.js.map