...
async function main() {
    ...
    const [optin] = await db.insert(website_builder_schema_1.wbGlobalTemplatePages).values({ templateId: t.id, name: 'Webinar Opt-in', slug: '/', isHomepage: true, order: 0 }).returning();
...
<p>In seiner Akademie hat er bereits <strong>847 Studenten</strong> dabei geholfen, ihre eigene finanzielle Freiheit zu erreichen.</p>`,
    ...
    const [sales] = await db.insert(website_builder_schema_1.wbGlobalTemplatePages).values({ templateId: t.id, name: 'Sales Page', slug: '/kurs', isHomepage: false, order: 1 }).returning();
    await db.insert(website_builder_schema_1.wbGlobalTemplateSections).values([
        {
...
//# sourceMappingURL=funnels_pro.js.map