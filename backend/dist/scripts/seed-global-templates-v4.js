...
const S = {
    ...
    custom: (bg, text) => ({ backgroundColor: bg, textColor: text }),
};
async function templateExists(name) {
    const r = await db.select({ id: website_builder_schema_1.wbGlobalTemplates.id }).from(website_builder_schema_1.wbGlobalTemplates)
        .where((0, drizzle_orm_1.eq)(website_builder_schema_1.wbGlobalTemplates.name, name)).limit(1);
    return r.length > 0;
}
async function deleteTemplate(name) {
    ...
    if (!r.length)
        return;
    await db.delete(website_builder_schema_1.wbGlobalTemplates).where((0, drizzle_orm_1.eq)(website_builder_schema_1.wbGlobalTemplates.id, r[0].id));
}
async function createTemplate(data, force) {
    if (await templateExists(data.name)) {
        if (!force) {
            console.log(`   ⏭️  "${data.name}" existiert — übersprungen`);
            return;
        }
        await deleteTemplate(data.name);
    }
    const [tpl] = await db.insert(website_builder_schema_1.wbGlobalTemplates).values({
        name: data.name, description: data.description, category: data.category,
        thumbnailUrl: data.thumbnailUrl, isActive: true, isPremium: data.isPremium,
        settings: data.settings,
    }).returning();
    for (const pg of data.pages) {
        const [page] = await db.insert(website_builder_schema_1.wbGlobalTemplatePages).values({
            templateId: tpl.id, name: pg.name, slug: pg.slug,
            isHomepage: pg.isHomepage, order: pg.order,
        }).returning();
        if (pg.sections.length) {
            await db.insert(website_builder_schema_1.wbGlobalTemplateSections).values(pg.sections.map(s => ({
...
async function main() {
    ...
    if (force)
        console.log('🔄 --force aktiv — vorhandene Templates werden überschrieben\n');
...
//# sourceMappingURL=seed-global-templates-v4.js.map