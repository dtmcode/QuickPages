...
async function createOrGetPrice(item, type) {
    ...
    const existing_price = prices.data.find(p => p.unit_amount === item.price && p.currency === 'eur' && p.recurring?.interval === 'month');
    let price;
    if (existing_price) {
...
//# sourceMappingURL=stripe-setup-products.js.map