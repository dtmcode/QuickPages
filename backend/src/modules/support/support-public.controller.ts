...
export class SupportPublicController {
  ...
  async createTicket(
    ...
    @Body() body: { subject: string; customerName: string; customerEmail: string; message: string; priority?: string },
  ) {
    const tenantId = await this.supportService.getTenantIdBySlug(slug);
    if (!tenantId) throw new HttpException('Nicht gefunden', HttpStatus.NOT_FOUND);
    if (!body.subject || !body.customerName || !body.customerEmail || !body.message) {
      throw new HttpException('Alle Pflichtfelder ausfüllen', HttpStatus.BAD_REQUEST);
    }
  ...
  @Get('ticket/:token')
  async getTicket(@Param('tenant') slug: string, @Param('token') token: string) {
    const ticket = await this.supportService.getTicketByToken(token);
    if (!ticket) throw new HttpException('Ticket nicht gefunden', HttpStatus.NOT_FOUND);
  ...
  async reply(
    ...
    if (!body.content) throw new HttpException('Nachricht darf nicht leer sein', HttpStatus.BAD_REQUEST);
    try {
      await this.supportService.addPublicMessage(token, body);
      return { success: true };
    } catch (err: unknown) {
      throw new HttpException(err instanceof Error ? err.message : 'Fehler', HttpStatus.BAD_REQUEST);
    }
  }
}