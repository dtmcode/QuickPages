// 📂 PFAD: backend/scripts/seed-global-templates-v3.ts
//
// Run:
//   npx ts-node -r tsconfig-paths/register scripts/seed-global-templates-v3.ts
//   npx ts-node -r tsconfig-paths/register scripts/seed-global-templates-v3.ts --force
//   npx ts-node -r tsconfig-paths/register scripts/seed-global-templates-v3.ts --category=website
...
// Newsletter → contact type mit { isNewsletter: true } im content
// Booking    → services type mit Terminen
...
async function deleteTemplate(name: string): Promise<void> {
  ...
  // Cascade löscht Pages + Sections automatisch
  await db.delete(wbGlobalTemplates).where(eq(wbGlobalTemplates.id, rows[0].id));
}
...
const TEMPLATES: TemplateDef[] = [
  ...
  // ╔══════════════════════════════════════════════════════╗
  // ║  KATEGORIE: WEBSITE                                  ║
  // ║  Paket-Mapping: website_micro / standard / pro       ║
  // ╚══════════════════════════════════════════════════════╝
  ...
  // ╔══════════════════════════════════════════════════════╗
  // ║  KATEGORIE: BLOG                                     ║
  // ║  Paket-Mapping: blog_personal / publisher / magazine ║
  // ╚══════════════════════════════════════════════════════╝
  ...
  // ╔══════════════════════════════════════════════════════╗
  // ║  KATEGORIE: BUSINESS                                 ║
  // ║  Paket-Mapping: business_starter / professional / agency ║
  // ╚══════════════════════════════════════════════════════╝
  ...
  // ╔══════════════════════════════════════════════════════╗
  // ║  KATEGORIE: SHOP                                     ║
  // ║  Paket-Mapping: shop_mini / wachstum / premium       ║
  // ╚══════════════════════════════════════════════════════╝
  ...
  // ╔══════════════════════════════════════════════════════╗
  // ║  KATEGORIE: MEMBERS                                  ║
  // ║  Paket-Mapping: members_community / kurse / academy  ║
  // ╚══════════════════════════════════════════════════════╝
...
async function main() {
  ...
  if (force)   console.log('🔄 --force: Vorhandene Templates werden überschrieben\n');
  ...
  for (const cat of ['website', 'blog', 'business', 'shop', 'members']) {
    const rows = await db
      .select({ id: wbGlobalTemplates.id })
      .from(wbGlobalTemplates)
      .where(eq(wbGlobalTemplates.category, cat));
    if (rows.length) console.log(`   ${cat.padEnd(12)} ${rows.length} Templates`);
  }
