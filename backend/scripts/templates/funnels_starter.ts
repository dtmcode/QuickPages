// 📂 PFAD: backend/scripts/templates/funnels_starter.ts
// Paket: funnels_starter — Immobilien Lead Funnel "ImmoGuide"
// Run: npx ts-node -r tsconfig-paths/register scripts/templates/funnels_starter.ts [--force]
...
async function main() {
  ...
  const [optin] = await db.insert(wbGlobalTemplatePages).values({ templateId: t.id, name: 'Lead Magnet', slug: '/', isHomepage: true, order: 0 }).returning();
  ...
  const [danke] = await db.insert(wbGlobalTemplatePages).values({ templateId: t.id, name: 'Danke', slug: '/danke', isHomepage: false, order: 1 }).returning();
  await db.insert(wbGlobalTemplateSections).values({
