// 📂 PFAD: backend/scripts/templates/restaurant_starter.ts
// Paket: restaurant_starter — Pizzeria "Da Marco", München
// Run: npx ts-node -r tsconfig-paths/register scripts/templates/restaurant_starter.ts [--force]
...
<p>Bei uns isst die ganze Familie: Kinder lieben unsere Junior-Pizzen, Erwachsene genießen unsere Weinauswahl aus kleinen italienischen Weingütern.</p>`,
  ...
  const [speisekarte] = await db.insert(wbGlobalTemplatePages).values({ templateId: t.id, name: 'Speisekarte', slug: '/speisekarte', isHomepage: false, order: 1 }).returning();
  await db.insert(wbGlobalTemplateSections).values([
