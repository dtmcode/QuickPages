// 📂 PFAD: backend/scripts/templates/website_micro.ts
//
// Demo-Template für Paket: website_micro
// Nische: Elektro Hoffmann — Elektriker in Köln
//
// Run:
//   npx ts-node -r tsconfig-paths/register scripts/templates/website_micro.ts
//   npx ts-node -r tsconfig-paths/register scripts/templates/website_micro.ts --force
...
async function main() {
  ...
  const [template] = await db.insert(wbGlobalTemplates).values({
    ...
    settings: {
      ...
      niche:    'handwerk',
      colors: {
        primary:    '#f97316',   // Kräftiges Orange — Handwerk, Energie
        secondary:  '#ea580c',
        accent:     '#fef3c7',
        background: '#ffffff',
        text:       '#1c1917',
  ...
  // ─── Startseite (One-Pager — alles auf einer Seite) ─────────────────────────
...
<p>Wir arbeiten ausschließlich nach DIN VDE 0100 und bieten <strong>5 Jahre Gewährleistung</strong> auf alle unsere Installationen.</p>`,
  ...
  const [impressum] = await db.insert(wbGlobalTemplatePages).values({
    templateId: template.id,
    name:       'Impressum',
    slug:       '/impressum',
    isHomepage: false,
...
> ⚠️ Bitte ersetze alle Angaben mit deinen echten Unternehmensdaten.`,
  ...
  const [datenschutz] = await db.insert(wbGlobalTemplatePages).values({
    templateId: template.id,
    name:       'Datenschutz',
    slug:       '/datenschutz',
    isHomepage: false,
