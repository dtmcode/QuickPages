import 'dotenv/config'; // ✅ HINZUFÜGEN!
...
async function seedGlobalTemplates() {
  ...
  // ==================== TEMPLATE 1: Business Pro ====================
  const [businessTemplate] = await db.insert(wbGlobalTemplates).values({
    name: 'Business Pro',
  ...
  // Homepage
  const [businessHome] = await db.insert(wbGlobalTemplatePages).values({
  ...
  const [creativeTemplate] = await db.insert(wbGlobalTemplates).values({
  ...
  const [creativeHome] = await db.insert(wbGlobalTemplatePages).values({
  ...
  const [ecomTemplate] = await db.insert(wbGlobalTemplates).values({
    name: 'E-Commerce',
  ...
  const [ecomHome] = await db.insert(wbGlobalTemplatePages).values({
  ...
  // ==================== TEMPLATE 4: Portfolio ====================
  const [portfolioTemplate] = await db.insert(wbGlobalTemplates).values({
    name: 'Portfolio',
  ...
  const [portfolioHome] = await db.insert(wbGlobalTemplatePages).values({
  ...
  const [restaurantTemplate] = await db.insert(wbGlobalTemplates).values({
    name: 'Restaurant',
  ...
  const [restaurantHome] = await db.insert(wbGlobalTemplatePages).values({
